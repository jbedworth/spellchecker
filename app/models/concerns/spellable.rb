...
## better factored out here. Plus, it was easier to test this functionality in isolation.
...
class Spellable
  ...
  def word_regex
    Spellable.compute_regex( @word )
  end

  def word_hash
    Spellable.compute_hash( @word )
  end

  def word_hash_regex
    Spellable.compute_hash_regex( @word )
  end
  ...
  def dictionary_word
    Spellable.clean_up( @word.downcase )
  end
